...
public class Program6 {
	public static void main(String[] args) {
		...
		value.stream().collect(Collectors.groupingBy(Student::getState)).entrySet().forEach(e -> {
			System.out.println(e.getKey() + ":-");
			e.getValue().stream().collect(Collectors.groupingBy(Student::getCity, Collectors.counting())).entrySet()
